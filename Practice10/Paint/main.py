# first run the code without activating the base_layer
# then activate the base_layer (uncomment 3 related lines of code) 
# and run the code again

import pygame
import math

pygame.init()

WIDTH = 800
HEIGHT = 600

screen = pygame.display.set_mode((WIDTH, HEIGHT))
base_layer = pygame.Surface((WIDTH, HEIGHT))


colorRED = (255, 0, 0)
colorBLUE = (0, 0, 255)
colorWHITE = (255, 255, 255)
colorBLACK = (0, 0, 0)
colorGREEN = (0,255,0)

color = colorRED
clock = pygame.time.Clock()

LMBpressed = False
THICKNESS = 5

currX = 0
currY = 0

prevX = 0
prevY = 0

mode = 0

def calculate_rect(x1, y1, x2, y2):
    return pygame.Rect(min(x1, x2), min(y1, y2), abs(x1 - x2), abs(y1 - y2))

def calculate_circ(screen,colorRED,start,end,radius):
    cx, cy = start
    ex, ey = end

    radius = int(math.hypot(ex - cx, ey - cy))

    if radius > 0:
        pygame.draw.circle(screen, colorRED, (cx, cy), radius, THICKNESS)

running = True

while running:

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            print("LMB pressed!")
            LMBpressed = True
            currX = event.pos[0]
            currY = event.pos[1]
            prevX = event.pos[0]
            prevY = event.pos[1]

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_q:
                mode = "2"
            elif event.key == pygame.K_r:
                mode = "1"
            elif event.key == pygame.K_e:
                mode = "3"
            elif event.key == pygame.K_1:
                print("Picked new color!")
                color = colorRED
            elif event.key == pygame.K_2:
                print("Picked new color!")
                color = colorBLUE
            elif event.key == pygame.K_3:
                print("Picked new color!")
                color = colorGREEN
            
        if event.type == pygame.MOUSEMOTION:
            screen.blit(base_layer,(0,0))
            print("Position of the mouse:", event.pos)
            if LMBpressed:
                if mode == "1":
                    currX = event.pos[0]
                    currY = event.pos[1]
                    pygame.draw.rect(screen, color, calculate_rect(prevX, prevY, currX, currY), THICKNESS)
                elif mode == "2":
                    currX = event.pos[0]
                    currY = event.pos[1]
                    calculate_circ(screen, color, (prevX, prevY), (currX,currY), THICKNESS)
                elif mode == "3":
                    currX, currY = event.pos
                    pygame.draw.line(base_layer, colorBLACK, (prevX, prevY), (currX, currY), THICKNESS)
                    prevX, prevY = currX, currY

                    screen.blit(base_layer,(0,0))
                   
        


        if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
            print("LMB released!")
            LMBpressed = False
            if mode == "1":
                pygame.draw.rect(screen, color, calculate_rect(prevX, prevY, currX, currY), THICKNESS)
                base_layer.blit(screen,(0,0))
            elif mode == "2":
                 calculate_circ(screen, color, (prevX, prevY), (currX,currY), THICKNESS)
                 base_layer.blit(screen,(0,0))
            elif mode == "3":
                pygame.draw.line(screen, color, (prevX, prevY), (currX, currY), THICKNESS)
                base_layer.blit(screen,(0,0))
                

        


        if event.type == pygame.KEYDOWN: 
            if event.key == pygame.K_EQUALS:
                print("increased thickness")
                THICKNESS += 1
            if event.key == pygame.K_MINUS:
                print("reduced thickness")
                THICKNESS -= 1

    pygame.display.flip()
    clock.tick(60)